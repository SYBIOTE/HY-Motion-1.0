...
Cloud Build uses a 3-step pipeline: pulls cached images, builds the shared base (`BUNDLE_CKPTS=1`: CUDA + deps + checkpoints baked into the image), then builds the thin service layer. Substitute `_BUNDLE_CKPTS=0` for slim images meant to load weights from mounted storage at runtime.

First bundled build takes ~20–30 min (PyTorch + model downloads); subsequent builds use cached layers.
